def nextPermutation(nums):
    ...
    while i > 0 and nums[i-1] >= nums[i]:
        i -= 1
    if i == 0:   # nums are in reverse and descending order if idx 0 is greatest
        nums.reverse()
        return nums
    k = i - 1    # find the last "ascending" position k 
    while nums[j] <= nums[k]: #<= since right side is already in descending order, compare j and k now
        j -= 1
...
if __name__ == '__main__':
    ...
    print(nextPermutation([3,4,2,1])) # 4123 i=1,j=1,4321,j=3,4123,j=2,i=2, 4321->4123
